...
- Consolidated all held posts that expire during the same hourly sweep into one protected review-channel notice per server, avoiding one expiry card per queue entry while preserving no-strike expiry semantics
...
- Replaced every Fandom-hosted icon in `emoji-icons.js` with a Game8-hosted one — Fandom's media-serving endpoint rejects automated requests with a blanket 403 in production, confirmed against a live run, while its regular wiki content pages are unaffected. Every URL now comes from a live Game8 reward icon and was verified reachable before being added
- Fixed the fallback icons themselves 404ing: they were reused from `GAMES[*].icon`, which had quietly rotted for HSR, ZZZ, and HI3. HSR's Adventure Log now falls back to Game8's HSR hub icon instead; HI3 has no reliable per-item or per-game icon source at all (no Game8 hub, blocked Fandom media, rotted HoYoLAB uploads) so its 6 keywords are left out of provisioning entirely rather than guessing another broken URL — they still render fine via Unicode. `GAMES.hkrpg.icon`/`GAMES.nap.icon` (used for the code-announcement embed icon, not just provisioning) are now Game8-hosted too; `GAMES.honkai3rd.icon` uses the official site's favicon as a lower-quality but working fallback
