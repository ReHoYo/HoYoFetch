...
export const COMMAND_CATALOG = Object.freeze([
  ...
  },
  {
    ...
    syntax: "/Auto-Fetch [status|enable|off]",
    summary:
      "Inspect or configure hourly code announcements in the current channel.",
    help: "Inspect, enable, or disable hourly code announcements in this channel _(admins/mods only)_",
    permission:
    ...
    notes: ["Omitting the enable scope selects every supported game."],
  },
  {
    id: "emoji",
    section: COMMAND_SECTIONS.SETUP,
    route: "emoji",
    access: "fetch_manager",
    syntax: "/Emoji [status|mode]",
    summary: "Inspect or configure reward emoji rendering.",
    help: "Inspect or configure reward emoji rendering _(admins/mods only)_",
    permission:
      "Server owner, Manage Server, or a recognized moderation capability",
    examples: ["/Emoji status", "/Emoji mode custom", "/Emoji mode unicode"],
  },
  {
    id: "emoji-provision",
    section: COMMAND_SECTIONS.SETUP,
    route: "emoji provision",
    access: "server_moderator",
    syntax: "/Emoji provision",
    summary:
      "Auto-provision reward icons as custom emoji on the configured hub server.",
    help: "Provision reward emoji on the configured hub server _(server admins/mods only)_",
    permission: "Server owner, Kick/Ban/Timeout Members, or admin",
    examples: ["/Emoji provision"],
    notes: [
      "Provisioning only runs in the server set as EMOJI_HUB_SERVER_ID — elsewhere it reports registry status without uploading anything.",
      ...
      "Switch rendering over to the result with `/Emoji mode custom`.",
    ],
  },
  {
    id: "audit-log",
    ...
    notes: [
      "Enabling, moving, or disabling audit logging and changing privacy exclusions requires a one-time code approved by Enka#4961.",
  ...
  },
  {
    ...
    examples: [
      ...
      "/Post-Gate hold @member repeated boundary-pushing after a warning",
    ],
    notes: [
      "Turning the gate on, moving its review channel, and turning it off each require a fresh one-time code sent to Enka#4961.",
      "Approving a held post (✅), or running `/Post-Gate release @member` (🔓), marks the author exempt from contact-solicitation, prohibited-identity, and first-post link/media screening going forward — the slur filter still applies — until a moderator manually holds them again with `/Post-Gate hold` or 🔒 Deny + Hold User. Release grants this even when the account has no active full hold to release.",
      ...
      "`/Post-Gate protection` owns the independent monitor/enforce detector, containment ladder, quorum, ban-case approval, and timeout release actions.",
  ...
  },
  {
    ...
    notes: [
      "Approving clears the author, resets their protection strike, and marks them permanently exempt from automatic contact-solicitation, prohibited-identity, and first-post link/media screening — the everyday way a member earns that exemption, since most held posts never place a full account hold. It does not repost the held content, which the author may post again themselves. The message-content prohibited-term (slur) filter still applies regardless.",
      "Rejecting discards the held content and increases the author's protection strike stage; it does not apply a timeout or grant any exemption.",
      "Deny + Hold User (🔒) also places the author in Post Gate, so every later message they send is held until a moderator releases them. A persistent control card in the review channel records who held them and when. If the author held a standing exemption from an earlier approval or release, this revokes it.",
      "Release (🔓 or `/Post-Gate release @member`) grants the same standing exemption approving does, whether or not the account currently has an active full hold — Post Gate's account-level hold and per-message queuing are independent layers, and most accounts only ever hit the second one. When there is an active hold, release also restores normal posting straight away; messages already in the review queue stay queued and are reviewed individually.",
      ...
      "Unreviewed held posts expire and are discarded without a strike after 7 days; one notice summarizes all entries expiring in the same hourly sweep. An expiry never grants the exemption.",
      "`/Post-Gate hold @member <reason>` manually places a member in full Post Gate without waiting for an automatic trigger, and revokes any standing exemption the same way Deny + Hold User does.",
    ],
  },
  {
    id: "post-gate-protection-release",
    section: COMMAND_SECTIONS.MODERATION,
    route: "post-gate protection release",
    access: "timeout",
    syntax: "/Post-Gate protection release @member <reason>",
    summary:
      "Release a timeout and reset that member's protection escalation history.",
    help: "Release the timeout immediately and reset that member's protection strikes _(Timeout Members)_",
    permission: "Timeout Members",
    examples: ["/Post-Gate protection release @member false positive"],
  },
  {
    id: "post-gate-protection",
    section: COMMAND_SECTIONS.MODERATION,
    route: "post-gate protection",
    access: "fetch_manager",
    syntax: "/Post-Gate protection [monitor|enforce|off|quorum]",
    summary: "Configure Post Gate behavioral anti-raid protection.",
    ...
    permission: "Configuration: recognized moderator capability",
    examples: [
      "/Post-Gate protection monitor here",
      "/Post-Gate protection enforce #protection-log",
      "/Post-Gate protection quorum 2",
      "/Post-Gate protection off",
    ],
    notes: [
      "Protection keeps its independent off/monitor/enforce setting and still requires message behavior plus a score of 2.",
      "Five non-bot joins in 60 seconds activate a persisted 30-minute Level 2 floor when either the review queue or protection is enabled; the join surge alone never opens a case or changes a member.",
    ],
  },
  {
    id: "post-gate-protection-approve",
    section: COMMAND_SECTIONS.MODERATION,
    route: "post-gate protection approve",
    access: "ban_approver",
    syntax: "/Post-Gate protection approve CASE_ID",
    summary: "Approve a pending Post Gate Protection ban case.",
    help: "Approve a pending protection ban case _(owner, Manage Server, or Ban Members)_",
    permission: "Server owner, Manage Server, or Ban Members",
    examples: ["/Post-Gate protection approve CASE_ID"],
  },
]);
