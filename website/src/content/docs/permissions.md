...
| Server owner or Manage Server                              | Request, confirm, cancel, or inspect `/Exclude-Channel`; the bot owner must also approve mutations |
...
Channel privacy exclusions are intentionally stricter than other audit configuration. Only the server owner or **Manage Server** may invoke `/Exclude-Channel`, and adding or removing an exclusion still requires the separate bot-owner one-time code.
