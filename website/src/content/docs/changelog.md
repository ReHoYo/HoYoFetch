...
- Replaced every Fandom-hosted icon in `emoji-icons.js` with a Game8-hosted one. Fandom's `Special:FilePath` media-serving endpoint rejects automated requests with a blanket HTTP 403 — confirmed against a live deployed run, not just guessed filenames — while its regular wiki _content_ pages (the ones the HI3 code scraper already reads) are unaffected. Every remaining URL was pulled from a live Game8 reward-icon `<img>` tag and verified with a HEAD request before being added. Honkai Impact 3rd (no active Game8 coverage) and Honkai: Star Rail's Adventure Log (no dedicated Game8 page) fall back to the game's own icon rather than a guessed filename that would just add another failure.
